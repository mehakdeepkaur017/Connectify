...
export default function RestrictedUsersPage() {
  ...
  const { data: restrictedUsers, isLoading } = useQuery({
    queryKey: ['restrictedUsers'],
    queryFn: async () => {
      const res = await api.get('/users/settings/restricted');
  ...
  const unrestrictMutation = useMutation({
    ...
    onSuccess: (userId) => {
      queryClient.invalidateQueries({ queryKey: ['restrictedUsers'] });
      if (user) {
        updateUser({
          restrictedUsers: user.restrictedUsers?.filter(id => id !== userId) || []
        });
      }
    }
  });
  ...
  return (
    <div className="p-8 max-w-2xl">
      <h1 className="text-2xl font-bold mb-6">Restricted Accounts</h1>
      ...
      {isLoading ? (
        ...
        <div className="text-center p-8 text-muted-foreground">You haven&apos;t restricted anyone.</div>
      ) : (
        <div className="space-y-4">
          {restrictedUsers?.map((user: { _id: string, username: string, fullName: string, avatar: string }) => (
            <div key={user._id} className="flex items-center justify-between p-4 rounded-xl border border-border">
              ...
              <Button 
                variant="secondary" 
                size="sm" 
                onClick={() => unrestrictMutation.mutate(user._id)}
